    ...
    fetch(`/api/blocks/${blockId}/`, {
        ...
        .then(data => {
            ...
            `;
                ...
                const autoplay = blockData.autoplay !== false; // по умолчанию true
                const interval = blockData.interval || 3000;
                
                // Инициализируем глобальный массив изображений
                if (!window.sliderImages) {
                    window.sliderImages = [];
                }
                window.sliderImages = [...images];
                
                const imagesList = images.length > 0 ? images.map((img, idx) => `
                    <div class="slider-image-item" data-index="${idx}" style="position: relative; border: 2px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
                        <img src="${img}" style="width: 100%; height: auto; display: block;" alt="Slide ${idx + 1}">
                        <button type="button" class="slider-remove-btn" onclick="removeSliderImage(${idx})" style="position: absolute; top: 5px; right: 5px; background: rgba(239, 68, 68, 0.9); color: white; border: none; border-radius: 50%; width: 30px; height: 30px; cursor: pointer; font-size: 16px;" title="Удалить">🗑️</button>
                    </div>
                `).join('') : '<p style="color: #6b7280; text-align: center; grid-column: 1/-1;">Нет изображений</p>';
                ...
                <form id="edit-form">
                    ...
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 0.5rem;">
                            <input type="text" id="field-image-url" class="form-input" placeholder="https://example.com/image.jpg">
                    ...
                    <div class="form-group">
                        <label class="form-label">
                            <input type="checkbox" id="field-autoplay" ${autoplay ? 'checked' : ''}>
                            Автопрокрутка
                        </label>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Интервал прокрутки (мс)</label>
                        <input type="number" id="field-interval" class="form-input" value="${interval}" min="1000" step="500">
                    </div>
                </form>
            `;
            }
        ...
        `;
            ...
            // Обработчик загрузки изображений для слайдера
            if (blockType === 'slider') {
                ...
                setTimeout(() => {
                    ...
                    if (fileInput) {
                        ...
                        fileInput.removeEventListener('change', fileInput._sliderHandler);
                        
                        // Создаем новый обработчик
                        fileInput._sliderHandler = async function(e) {
                            ...
                            if (!list) {
                                console.error('Элемент slider-images-list не найден');
                                return;
                            }
                            
                            const originalContent = list.innerHTML;
                            list.innerHTML = '<p style="color: #6b7280; text-align: center; grid-column: 1/-1;">Загрузка изображений...</p>';
                            ...
                            for (const file of files) {
                                try {
                                    ...
                                    const response = await fetch(`/api/blocks/${blockId}/upload-slider-image/`, {
                                        method: 'POST',
                                        headers: {
                                            'X-CSRFToken': getCookie('csrftoken')
                                        },
                                    ...
                                    if (result.success && result.image_url) {
                                        window.addSliderImageToList(result.image_url);
                                        successCount++;
                                    } else {
                            ...
                            if (successCount > 0) {
                                console.log(`Успешно загружено ${successCount} из ${files.length} изображений`);
                            }
                        ...
                        fileInput.addEventListener('change', fileInput._sliderHandler);
                        console.log('Обработчик загрузки изображений установлен');
                    } else {
                        console.error('Элемент field-image не найден');
                    }
                }, 100);
            }
        })
...
async function saveEditedBlock(blockId, blockType) {
    ...
    if (blockType === 'text') {
        ...
        // Сохраняем изображения слайдера
        if (!window.sliderImages) {
            window.sliderImages = [];
        }
        newData.images = window.sliderImages;
        console.log('Сохранение слайдера:', {
            images: newData.images,
            count: newData.images.length
        });
        
        // Сохраняем настройки автопрокрутки
        const autoplayEl = document.getElementById('field-autoplay');
        newData.autoplay = autoplayEl ? autoplayEl.checked : true;
        
        // Сохраняем интервал
        const intervalEl = document.getElementById('field-interval');
        newData.interval = intervalEl ? parseInt(intervalEl.value) || 3000 : 3000;
        
        console.log('Настройки слайдера:', {
    ...
    // Инициализируем слайдеры после обновления
    if (blockType === 'slider') {
        setTimeout(() => {
            initEditorSliders();
        }, 500);
...
// === ФУНКЦИИ ДЛЯ РАБОТЫ С ИЗОБРАЖЕНИЯМИ СЛАЙДЕРА ===
// Делаем функции глобальными для доступа через onclick
window.addSliderImageToList = function(imageUrl) {
    console.log('Добавление изображения:', imageUrl);
    if (!window.sliderImages) {
        window.sliderImages = [];
    }
    window.sliderImages.push(imageUrl);
    console.log('Текущие изображения:', window.sliderImages);
    updateSliderImagesList();
};

window.removeSliderImage = function(index) {
    if (!window.sliderImages) return;
    window.sliderImages.splice(index, 1);
    updateSliderImagesList();
};

window.addSliderImageFromUrl = function() {
    const urlInput = document.getElementById('field-image-url');
    if (urlInput && urlInput.value.trim()) {
        window.addSliderImageToList(urlInput.value.trim());
...
function updateSliderImagesList() {
    const list = document.getElementById('slider-images-list');
    if (!list) {
        console.warn('Элемент slider-images-list не найден');
        return;
    }
    ...
    if (label && label.classList.contains('form-label')) {
        const count = window.sliderImages ? window.sliderImages.length : 0;
        label.textContent = `Изображения в слайдере (${count})`;
    }

    if (!window.sliderImages || window.sliderImages.length === 0) {
        list.innerHTML = '<p style="color: #6b7280; text-align: center; grid-column: 1/-1;">Нет изображений</p>';
        return;
    }

    list.innerHTML = window.sliderImages.map((img, idx) => `
        <div class="slider-image-item" data-index="${idx}" style="position: relative; border: 2px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
            <img src="${img}" style="width: 100%; height: auto; display: block;" alt="Slide ${idx + 1}" onerror="this.style.display='none'; this.parentElement.innerHTML='<p style=\\'color: red; padding: 10px;\\'>Ошибка загрузки изображения</p>';">
            <button type="button" class="slider-remove-btn" onclick="removeSliderImage(${idx})" style="position: absolute; top: 5px; right: 5px; background: rgba(239, 68, 68, 0.9); color: white; border: none; border-radius: 50%; width: 30px; height: 30px; cursor: pointer; font-size: 16px;" title="Удалить">🗑️</button>
        </div>
    `).join('');
}
...
window.changeSlide = function(sliderId, direction) {
    ...
    window.goToSlide(sliderId, currentIndex);
};
...
function startSlider(container) {
    ...
    container.sliderInterval = setInterval(() => {
        ...
        window.goToSlide(container.id, currentIndex);
    }, interval);
}
